...
def test_pageindex_navigation_top_3():
    ...
    root1 = PageIndexNode(section_id="1", title="Capital Expenditure Outlook", summary="CAPEX plans and future scaling.")
    root2 = PageIndexNode(section_id="2", title="Operating Expenses", summary="OPEX day to day costs.")
    root3 = PageIndexNode(section_id="3", title="Revenue Streams", summary="Where the money comes from.")
    root4 = PageIndexNode(section_id="4", title="Risk Factors", summary="What could go wrong.")
    ...
    def mock_navigate(nodes, topic, k=3):
        ...
        sorted_nodes = sorted(nodes, key=lambda n: topic.lower() in n.title.lower(), reverse=True)
    ...
    assert sections[0].title == "Capital Expenditure Outlook", "Failed to retrieve the correct relevant node at rank 1."
